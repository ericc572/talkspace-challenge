...
def _assert_all_messages(base_url, cert_path, expected_digests=None):
    url = '{}/all-messages'.format(base_url)
    print(url)
    r = requests.get(url=url, verify=cert_path)
    assert r.status_code == 200, r.text
    resp_json = r.json()
    assert 'digestList' in resp_json, 'digestList not in json: {}'.format(resp_json)
    returned_digest = resp_json['digestList']
    if expected_digests:
        assert returned_digest == expected_digests, r.text
    print('{} listed successfully'.format(url))
    return returned_digest
...
if __name__ == '__main__':
    ...
    # GET all message hashes
    _assert_all_messages(
        base_url=BASE_URL,
        cert_path=args.cert_path,
        expected_digests=[
            digest_returned,
